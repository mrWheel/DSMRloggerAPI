...
//-- moved to arduinoGlue.h // #define   MAXLINELENGTH     128   // longest normal line is 47 char (+3 for \r\n\0)
...
char telegram[MAX_TLGRM_LENGTH] = "";
...
//-- moved to arduinoGlue.h // #define   TELEGRAM_INTERVAL   5 //seconds
...
void handleTestdata()
{
  ...
  if (!forceBuildRingFiles && (!DUE(telegramTimer)))
    return; // not yet time for new Telegram
  ...
  if (settingPreDSMR40 == 1)
    ...
    res = P1Parser::parse(&DSMRdata, telegram, lengthof(telegram), false, true);
  ...
  {
    sprintf(cMsg, "%02d%02d%02d%02d%02d%02dW\0\0", (year(nt) - 2000), month(nt), day(nt), hour(nt), minute(nt), second(nt));
...
int16_t buildTelegram(int16_t line, char telegramLine[])
{
  ...
  case 0:
    sprintf(telegramLine, "/XMX5LGBBLB2410065887\r\n");
    break;
  case 1:
  ...
  case 2:
    ...
    break;
  case 3:
    sprintf(telegramLine, "0-0:1.0.0(%12.12sS)\r\n", newTimestamp);
    break;
  case 4:
  ...
  case 41: // [4] Device-Type
    sprintf(telegramLine, "0-4:24.1.0(003)\r\n", val);
    break;
...
int16_t buildTelegram30(int16_t line, char telegramLine[])
{
  ...
  */
  ...
  case 0:
    sprintf(telegramLine, "/KMP5 KA6U001585575011\r\n");
    break;
  case 1:
    sprintf(telegramLine, "\r\n");
    break;
...
void updateMeterValues(uint8_t period)
{
  ...
  **/
  ...
  if (Verbose2)
    Debugf("l1[%5d], l2[%5d], l3[%5d] ==> l1+l2+l3[%9.3f]\n", (int)(IPD_l1 * 1000), (int)(IPD_l2 * 1000), (int)(IPD_l3 * 1000), PDelivered);
